...
constexpr uint32_t PANIC_BLACKOUT_MS = 1;  //Blackout (I halen på panic_MS længde) UPDATED 24 juli
...
// ----------------- Stage 2 params ----------------- UPDATED 24 juli
constexpr CRGB stage2_groupColor = CRGB(255, 10, 150);
constexpr uint8_t stage2_initBrightness = 25;  // 0..100
constexpr uint8_t stage2_brightnessPct = 80;   // 0..100
constexpr uint8_t stage2_basePct = 20;         // 0..100
constexpr uint8_t stage2_groupSize = 20;
constexpr uint16_t stage2_msPerLed = 30;
constexpr uint16_t stage2_trailFade = 10;
...
constexpr uint8_t stage3_sparkleBrightnessPct = 60;  // 0..100
...
constexpr uint8_t stage4_minBrightnessPct = 10;  // 0..100
constexpr uint8_t stage4_maxBrightnessPct = 80;  // 0..100
    ...
    case 2:
      {
        ...
        if (v > vCap) v = vCap;
        fillAll(stage2_groupColor);
        nscale8_video(leds, NUM_LEDS, v);
        FastLED.show();
      }
      break;
